...
class Student{
	...
	*/
	render(){
		...
		this.domElements.operations = $('<td>').append(deleteButton);
		$(deleteButton).on('click', this.handleDelete);

		var rowElements = [this.domElements.name, this.domElements.course, this.domElements.grade, this.domElements.operations];

		for(var data = 0; data<rowElements.length; data++) {
			this.domElements.row.append(rowElements[data]);
		}
